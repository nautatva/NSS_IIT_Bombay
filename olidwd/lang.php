...
?>
        ...
        <?php
            ...
            $conn= mysqli_connect($host, $username, $password, $db);
            if(!isset($_GET["page"])){
              $sql = "SELECT * FROM OLI WHERE LANG='".$_GET['l']."'";
            	// echo $sql;
              // echo "Here";
            	$result = mysqli_query($conn, $sql);
            ...
            $offset = $page * 6;
            $sql = "SELECT * FROM OLI WHERE LANG='".$_GET['l']."' LIMIT $offset, 6";
            $result = mysqli_query($conn, $sql);
            // echo $sql;
            while ($row = mysqli_fetch_assoc($result)) {  
            ...
            echo '
                        ...
                        <div class="s-12 m-12 l-6 ">
                         ...
                         ';
                         if($page!=0)
                         echo '<a href="lang.php?l='.$_GET["l"].'&page='.($page - 1).'&t='.$rowcount.'"><button class="button background-primary border-radius text-white text-strong">< Previous Page </button></a>';
                       echo'
                     </div>
                     </div>
                     <div class="s-12 m-12 l-6 ">
                         ...
                         echo '<a href="lang.php?l='.$_GET["l"].'&page='.($page + 1).'&t='.$rowcount.'"><button class="button background-primary border-radius text-white text-strong"> Next Page > </button></a>';
                       echo'
                     </div>
                     </div>
                       </div>
