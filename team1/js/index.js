...
draggable.addEventListener("drag", function() { 
  ...
  info.style.textShadow = "0px 0px " + (Math.abs(displayY - pointsArr[i].y)*1.5) / 20 + "px #000000";
  Headername.style.textShadow = "0px 0px " + (Math.abs(displayY - pointsArr[i].y)*1.5) / 20 + "px #000000";
...
function closestPoint(x, y) {
  ...
  for (var i = 0; i < pointsArr.length; i++) {
    var D = Math.sqrt(Math.pow((pointsArr[i].x - x), 2) + Math.pow((pointsArr[i].y - y), 2));
    distanceArr[i] = D;
  };
